export const Canvas3D = {
  ...
  createCanvasTexture: function ({ wh, color }) {
    ...
    const texture = new this._d3.THREE.CanvasTexture(canvas);
      ...
      sphere: v =>
        new THREE.Mesh(
          ...
          new THREE.MeshStandardMaterial({
            ...(v.map !== undefined && { map: v.map }),
            color: v.color,
            transparent: v.opacity !== undefined,
            opacity: v.opacity ?? 1.0
          })
        ),
      // 位置はboxの中心、szは1辺の長さ、よってsz=2の場合、中心の左右に1ずつある
      box: v =>
        new THREE.Mesh(
          new THREE.BoxGeometry(...v.sz.px2world_sz()),
          new THREE.MeshStandardMaterial({
  ...
  init_canvas: async function (ref_rect_elem, options = {}) {
    ...
    this._d3.renderer.domElement.style.zIndex = 5;
    ...
    // debug
    this._d3.scene.add(new THREE.AmbientLight(0xffffff, 1.0));

    const debug_light = [
      new THREE.DirectionalLight(0xffffff, 1.0),
      e => e.position.set(0, 0, 1),
      e => e.target.position.set(0, 0, 0),
    ].a2e();
    this._d3.scene.add(debug_light);

    // debug_light.position.set(debug_light.position.x,debug_light.position.y,5.16525754857469)

    // console.log(this._d3.camera.userData.position)
    // console.log(debug_light.position)
    // console.log(debug_light.target.position)

    // const helper = new THREE.CameraHelper(debug_light.shadow.camera);
    // this._d3.scene.add(helper);
    // debug_light.shadow.camera.position.set(debug_light.position.x,debug_light.position.y,debug_light.position.z)
    // debug_light.shadow.camera.lookAt(debug_light.position.x,debug_light.position.y,0)
    // console.log(debug_light.shadow.camera)

    // let debug_light_range_direction = -1;
    // setInterval(() => {
    //   const z1 = Math.max(debug_light_range[0], Math.min(debug_light_range[1],
    //     debug_light.position.z + (debug_light_range_direction * this.get_ThreeJS_scale() * 10000)
    //   ));
    //   if (z1 === debug_light_range[0] || z1 === debug_light_range[1]) {
    //     debug_light_range_direction *= -1;
    //   }
    //   console.log(z1 / this.get_ThreeJS_scale());
    //   debug_light.position
    //   debug_light.position.set(
    //     debug_light.position.x,
    //     debug_light.position.y,
    //     z1
    //   )
    // }, 100)
    ...
    if (options.background !== undefined) {
      if (/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(options.background)) {
        ...
        this._d3.scene.background = new THREE.TextureLoader().load(options.background);
      }
    }
  ...
  raycast: function ({ coords, threejs_objects } = {}) {
    this._d3.raycaster ??= new this._d3.THREE.Raycaster();
    this._d3.raycaster_coords ??= new this._d3.THREE.Vector2();
    threejs_objects ??= this._d3.scene.children;

    this._d3.raycaster_coords.x = coords[0];
    this._d3.raycaster_coords.y = coords[1];
    this._d3.raycaster.setFromCamera(this._d3.raycaster_coords, this._d3.camera);
    return this._d3.raycaster.intersectObjects(threejs_objects);
  },
  ...
  _picking: {
    ...
    start: function (ev, _d3) {
      // this <- _picking
      const rect = _d3.renderer.domElement.getBoundingClientRect();
      this.pointer_scene.x = (ev.offsetX * 2) / rect.width - 1;
      this.pointer_scene.y = -((ev.offsetY * 2) / rect.height) + 1;
      this.raycaster.setFromCamera(this.pointer_scene, _d3.camera);

      const intersectObject = this.raycaster.intersectObjects(_d3.scene.children)[0];
    ...
    move: function (ev) {
      this.target?.userData.set_pos({ p: [ev.offsetX, ev.offsetY, 0] })
  ...
  _set_pointer_event: async function (elem) {
    ...
    function translate_ev(ev) {
      const ev2 = new Proxy(ev, {
        get(tag, prop) {
          if (prop === 'clientX' || prop === 'clientY') {
            return tag[prop] + ev2[`${prop}_offset`];
          } else if (prop === 'offsetX' || prop === 'offsetY') {
            // pointerdownイベントのoffsetXは、イベントが発生した要素の左上からの相対的な位置を示します。しかし、canvas3dのような透過的なキャンバスは、その下の要素がクリックされたときにイベントを発火させます。そのため、offsetXはキャンバスの相対位置ではなく、クリックされた下の要素の相対位置を返してしまいます。このコードは、offsetXの値を計算し直すことで、あたかもキャンバス上でイベントが起きたかのように見せるためのものです。
            return ev2[`client${prop[prop.length-1]}`] - ev2[`${prop}_for_elem`];
          }
          const value = tag[prop];
          if (typeof value === 'function') {
            return value.bind(tag);
          }
          return value;
        }
      })
      const iframe = iframes.find(iframe => iframe.contentDocument === ev.currentTarget);
      const elem_rect = elem.getBoundingClientRect();
      [ev2.offsetX_for_elem, ev2.offsetY_for_elem] = [elem_rect.left, elem_rect.top];
      if (iframe !== undefined) {
        const iframe_rect = iframe.getBoundingClientRect();
        ev2.clientX_offset = iframe_rect.left - elem_rect.left;
        ev2.clientY_offset = iframe_rect.top - elem_rect.top;
      } else {
        ev2.clientX_offset = 0;
        ev2.clientY_offset = 0;
      }
      return ev2;
    }
    function handleStart(ev) {
      const ev2 = translate_ev(ev);
      isPointerDown = true;
      isPanning = false;
      ...
      currentX = startX;
      currentY = startY;
      if (undefined !== this._picking.start(ev2, this._d3)) {
        (ev.currentTarget.documentElement || ev.currentTarget).setPointerCapture(ev2.pointerId);
      }
    }
    function handleMove(ev) {
      ...
      currentX = ev2.offsetX;
      currentY = ev2.offsetY;
    ...
    // キャプチャ時にstopPropagationでイベントを止めることにより、threejsの
    // canvasの下にあるhtml要素の発火を防ぐ。
    // また、pointerdown時にsetPointerCaptureでpointerイベントを束縛することのよって、
    // pointerupも同じelemから発せられるようになり、up,moveともにthreejs canvas
    // の下に隠れているhtml要素の発火を防ぐ。
    //
    // 2025.09.07 ここでbodyのキャプチャフェーズでstopPropagationしていたので、
    // screen.jsのボタンが押せなくなる現象が発生する
    // elem.addEventListener('pointerdown', handleStart.bind(this), true);

    elem.addEventListener('pointerdown', handleStart.bind(this));
    elem.addEventListener('pointermove', handleMove.bind(this));
    elem.addEventListener('pointerup', handleEnd.bind(this));
    elem.addEventListener('pointercancel', handleEnd.bind(this));
    const iframes = Array.from(document.querySelectorAll("iframe"));
    iframes.forEach(iframe => {
      iframe.addEventListener('load', () => {
        if (iframe.contentDocument) {
          // iframe.contentDocument.addEventListener('pointerdown', handleStart.bind(this), true)
          iframe.contentDocument.addEventListener('pointerdown', handleStart.bind(this));
          iframe.contentDocument.addEventListener('pointermove', handleMove.bind(this));
          iframe.contentDocument.addEventListener('pointerup', handleEnd.bind(this));
          iframe.contentDocument.addEventListener('pointercancel', handleEnd.bind(this));
          // console.log("set event listener in iframe")
        }
      })
    })
  },
