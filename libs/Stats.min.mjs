// nakada created 2024.10.12
export const createStats = function (elem) {
    const stats = new Stats();
    // 更新頻度の高い独自のFPSを表示するパネルを追加する。描画は、stats.customFpsPanel.updateにより
    // fps値は独自に計算したものを与える。
    stats.customFpsPanel = stats.addPanel(new Stats.Panel('FPS', '#0ff', '#002'));
    stats.showPanel(stats.domElement.children.length - 1);
    // const parent = document.getElementById("capture_target_contents");
    stats.domElement.style.cssText += ";display:inline-block";
    elem.appendChild(stats.domElement);

    const panel = document.createElement("div");
    panel.style.cssText = "display:inline-block;";
...
Stats.Panel = function (e, t, l) {
    var n = 1 / 0,
        a = 0,
        i = Math.round,
        o = 1, // nakada i(window.devicePixelRatio || 1),
        r = 80 * o,
        f = 48 * o,
        c = 3 * o,
        d = 2 * o,
        ...
        S.fillRect(s, p, u, m),
    {
        ...
        update: function (f, v) {
            n = Math.min(n, f),
                ...
                S.fillRect(s + u - o, p, o, m), S.fillStyle = l,
                S.globalAlpha = .9, S.fillRect(s + u - o, p, o, i((1 - f / v) * m))
        }
    }
};
